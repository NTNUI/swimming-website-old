...
function print_reset_and_submit_buttons(){
  global $t;
	print('<input onclick="mergePhoneNumberAndPrefix()" name="utfylt" type="submit" value="' . $t->get_translation("submit") . '" style="float: right;">');
...
?>
	...
	<form action="enrollment_reception_test" method="post">
...
<?php
  ...
  print_textBox("oldClub", "text", "gammelKlubb");
	...
	?>
  <input hidden name="phone-number-prefix" type="text" class="phoneNumberPrefixPlaceholder"></input>
...
<script type="text/javascript">
  function getPhonePrefix(){
    return document.getElementsByClassName("selected-dial-code")[0].innerText;
  }
  
  function setPhoneNumberPrefix(prefix){
    document.getElementsByClassName("phoneNumberPrefixPlaceholder")[0].innerText = prefix;
  }
  
  function mergePhoneNumberAndPrefix(){
    setPhoneNumberPrefix(getPhonePrefix);
  }

...
function connect($database)
{
	...
	if (!$database) {
		log_message("Failed to retrieve database name", __FILE__, __LINE__);
		return false;
	}
	...
	if (!$mysqli->set_charset("utf8")) {
		log_message("Failed to set utf8 charset", __FILE__, __LINE__);
		return false;
	}
...
function exec_sqli($server, $sql, $bind_params, &...$vars)
{
	...
	if (!$query) {
		log_message("Failed to prepare querry", __FILE__, __LINE__);
		die();
	}
